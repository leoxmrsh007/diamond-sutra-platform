/**
 * 六祖坛经种子数据
 * 六祖坛经（Platform Sutra）是中国禅宗的重要经典
 * 全名《六祖大师法宝坛经》，简称《坛经》
 */
  ...
  chapters: [
    ...
    },
    {
      ...
      sections: [
        ...
        { sectionNum: 2, heading: '六根忏悔', content: '善知识！既归依自性三宝，当各各于自心，深信自性本来清净，于一切法，不生执著。六根供养，忏悔已，普愿法界众生，同生净土。' },
    ...
    },
    {
      ...
      sections: [
        ...
        { sectionNum: 2, heading: '薛简问法', content: '薛简曰：京城禅德皆云：欲得会道，必须坐禅习定。若不因禅定而得解脱者，未之有也。未审师意如何？师曰：道由心悟，岂在坐也？' },
    ...
    },
    {
      ...
      sections: [
        ...
        { sectionNum: 5, heading: '最后教导', content: '师曰：汝等好住，吾灭度后，莫作世情哭泣往来，受著缞麻，非吾弟子，亦非正法。但识自本心，见自本性，无动无静，无生无灭，无去无来，无是无非，无住无往。' },
 ...
 * 种植六祖坛经数据
 */
export async function seedPlatformSutra() {
  console.log('开始种植六祖坛经数据...')

  // 检查是否已存在
  const existingSutra = await prisma.sutra.findUnique({
    where: { slug: PLATFORM_SUTRA_DATA.sutra.slug },
  })

  if (existingSutra) {
    console.log('六祖坛经已存在，跳过种植')
    return existingSutra
  }
  ...
  for (const chapterData of PLATFORM_SUTRA_DATA.chapters) {
    ...
    console.log(`  创建章节: 第${chapterData.chapterNum}品 ${chapterData.title}`)
    ...
    console.log(`    创建 ${chapterData.sections.length} 个段落`)
  }

  // 添加六祖坛经相关的佛学概念
  const concepts = [
    { name: '菩提自性', description: '惠能提出的核心思想，认为每个人的自性本来清净，具足万法，只需直了自性即可成佛。' },
    { name: '无念', description: '禅宗修行的核心方法，于念而无念，不执著于任何念头，保持心灵的自由与清净。' },
    { name: '一行三昧', description: '于一切时中，行住坐卧，常行一直心，是名一行三昧。' },
    { name: '顿悟', description: '惠能南宗禅的核心思想，认为可以直指人心，顿悟成佛，不需要经过漫长的渐修过程。' },
    { name: '定慧一体', description: '定与慧不是二法，而是一体的两个方面。即定之时慧在定，即慧之时定在慧。' },
    { name: '自性三宝', description: '佛者觉也，法者正也，僧者净也。自心皈依觉、正、净，是名自性三宝。' },
    { name: '本来无物', description: '惠能偈语"菩提本无树，明镜亦非台。本来无一物，何处惹尘埃"，表达空性智慧。' },
  ]
  ...
  console.log(`添加 ${concepts.length} 个佛学概念`)
  console.log('六祖坛经数据种植完成!')
