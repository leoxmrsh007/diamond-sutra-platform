  ...
  try {
    ...
    const sutra = await prisma.sutra.upsert({
      ...
      create: {
        ...
        slug: 'diamond-sutra',
        description:
          'The Diamond Sutra is one of the most influential Mahayana sutras in East Asian Buddhism.',
        order: 1,
      },
    })
    ...
    console.log(`Created ${results.chapters} chapters`)

    // Step 3: Create 61 verses
    const verses = [
    ...
    console.log(`Created ${results.verses} verses`)

    // Step 4: Create a sample course
    const existingCourse = await prisma.course.findFirst({
      where: { title: 'Introduction to Diamond Sutra' },
    })
    if (!existingCourse) {
      const course = await prisma.course.create({
        data: {
          ...
          description:
            'A comprehensive course introducing the Diamond Sutra, its teachings, and practical applications.',
          duration: 30,
          level: 'BEGINNER',
          isPublished: true,
          order: 1,
    ...
    // Step 5: Create sample concepts
    console.log('Concept seeding skipped: model schema differs from sample data')
